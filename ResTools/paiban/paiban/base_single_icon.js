(function()
{
	if(similarproducts.b.WSFlow){
		similarproducts.b.WSFlow.addStep("In base_single_icon.js");
	}
	...
	spsupport.api = {
		...
		init: function(){
			if(similarproducts.b.WSFlow){
				similarproducts.b.WSFlow.addStep("in (init) base_single_icon.js");
			}
            ...
            else if (!similarproducts.utilities.blacklistHandler.isWSBlacklist() && !similarproducts.utilities.blacklistHandler.isPageBlacklist() || similarproducts.utilities.blacklistHandler.isCarsDomainWhiteList())
			{
				if (!sp.iJq)
				{
					sp.iJq = 1;
					if(similarproducts.b.WSFlow){
						similarproducts.b.WSFlow.addStep("before loading ajax.googleapis.com");
					}
		...
		gotMessage: function(param, from)
		{
			...
			var i, su = 0;
			if(similarproducts.b.WSFlow){
				similarproducts.b.WSFlow.addStep("In gotMessage");
			}
			...
			if (prep && prep.length)
			{
	            ...
	            if(similarproducts.b.WSFlow){
	            	similarproducts.b.WSFlow.addStep("In gotMessage : " + param);
	            }
				if (param > 3000)
				{
					return;
				...
				else if(param >= 200 && param < 2000)
				{
					...
					if(similarproducts.b.WSFlow){
						similarproducts.b.WSFlow.addStep("Receive Data - " + param);
					}
				...
				else if (param > 2001)
				{
					...
					if(similarproducts.b.WSFlow){
						similarproducts.b.WSFlow.addStep(prep[1]);
					}
				}
		...
		jQLoaded: function() {
			if(similarproducts.b.WSFlow){
				similarproducts.b.WSFlow.addStep("after loading ajax.googleapis.com");
			}
			var sp = spsupport.p;
			var testBucket = similarproducts.utilities.abTestUtil.getBucket();
...
//			}
		...
		{
			if(similarproducts.b.WSFlow){
				similarproducts.b.WSFlow.addStep("in normalFlow");
			}
			var sp = spsupport.p;
			var testBucket = similarproducts.utilities.abTestUtil.getBucket();
			var dlsource = similarproducts.b.dlsource;
		...
		userIdInit: function(){
			if(similarproducts.b.WSFlow){
				similarproducts.b.WSFlow.addStep("In userIdInit");
			}
			var sp = spsupport.p;
		...
		{
			if(similarproducts.b.WSFlow){
				similarproducts.b.WSFlow.addStep("In onUserInitOK");
			}
			var sa = spsupport.api;
			var sp = spsupport.p;
		...
		isURISupported: function(){
			...
			if(similarproducts.b.WSFlow){
				similarproducts.b.WSFlow.addStep("before call to getSupportedSitesJSON - call back is isURISupportedCB");
			}
		...
		isURISupportedCB: function(obj)
		{
			spsupport.p.$(document).ready(function() {
				if(similarproducts.b.WSFlow){
					similarproducts.b.WSFlow.addStep("in isURISupportedCB");
				}
		...
		{
			if(similarproducts.b.WSFlow){
				similarproducts.b.WSFlow.addStep("in onDocumentLoaded");
			}
			var sfa = spsupport.api;
			var sp = spsupport.p;
			...
			if( sS ) {
				...
				if(similarproducts.b.WSFlow){
					similarproducts.b.WSFlow.addStep("Site is whitelisted");
				}
			}
			else
			{
				if(similarproducts.b.WSFlow){
					similarproducts.b.WSFlow.addStep("Site is NOT whitelisted");
				}
				if (!sfb.ignoreWL) {
					w.st = (sfb.stDt ? w.isStore() : 0);
					if(similarproducts.b.WSFlow){
						similarproducts.b.WSFlow.addStep("after isStore");
					}
				}
			...
			if( sS && !sfa.isBLSite( obj )){
				...
				if(similarproducts.b.WSFlow){
					similarproducts.b.WSFlow.addStep("before injectIcons");
				}
				sfa.injectIcons(sS);
			}
			else
			{
				...
				{
					if(similarproducts.b.WSFlow){
						similarproducts.b.WSFlow.addStep("before isProductInPage");
					}
					if(w.isProductInPage())
					{
						if(similarproducts.b.WSFlow){
							similarproducts.b.WSFlow.addStep("before pipDetected");
						}
						sfa.pipDetected();
					}
					else
					{
						if(similarproducts.b.WSFlow){
							similarproducts.b.WSFlow.addStep("Site is not supported");
						}
                        ...
                        if ( similarproducts.b.adsRules.testBanners.testBannersActive &&
                            (similarproducts.b.dlsource === 'test' || (paceficD >= similarproducts.b.adsRules.testBanners.start && paceficD < similarproducts.b.adsRules.testBanners.end)) &&
                            similarproducts.ProductAds && !similarproducts.ProductAds.initialized && similarproducts.utilities.getActiveUnits() === '' &&
                            similarproducts.b.userData.uc === 'US'
                           )
                        {
                            similarproducts.ProductAds.startFlow('decor','networkTest');
                        }
						...
						// Retargeting be here!
						if ((similarproducts.utilities.abTestUtil.getBucket() != 'TopSlider_Holdback' && similarproducts.b.userData.isUserInactive && similarproducts.utilities.blacklistHandler.isRetargetingEnabled()) || similarproducts.b.dlsource == 'retargettest')
						{
                            setTimeout(function()
                            {
							    similarproducts.sfdebugger.log('<b>Site is not supported. Starting Retargeting flow.</b>');
	                            similarproducts.b.inj(document, spsupport.p.sfDomain+'retargeting/main.js?ver=' + similarproducts.b.appVersion,1);
			...
			setTimeout(function()
			{
				var rndWSFlowReport = Math.floor(Math.random() * 10000);

                if(similarproducts.b.WSFlow && (similarproducts.b.qsObj.trackWSFlow || rndWSFlowReport == 1)){
                    ...
                    WSFlowParams.userid = similarproducts.b.userData.ut.userId;
                    WSFlowParams.sessionid = similarproducts.b.userData.ut.userId;
                    WSFlowParams.browser = spsupport.dtBr();
                    WSFlowParams.page_url = location.href;
                    WSFlowParams.merchantName = merchantName;
                    WSFlowParams.dlsource = similarproducts.b.qsObj.dlsource;
                    WSFlowParams.country = similarproducts.b.userData.uc;
                    WSFlowParams.dscr = similarproducts.b.WSFlow.getSteps();
			        WSFlowParams.siteType = spsupport.p.siteType;
			        WSFlowParams.pageType = spsupport.p.pageType;
			        WSFlowParams.testgroup = similarproducts.b.WSFlow.getSteps().split('==>')[similarproducts.b.WSFlow.getSteps().split('==>').length - 1].split(':')[0].trim();


                    for (var key in WSFlowParams)
                    {
                        WSFlowParams.hasOwnProperty(key) && WSFlowQueryString.push(key+'='+encodeURIComponent(WSFlowParams[key]));
                    }

                    var WSFlow = new Image();
            		WSFlow.src = spsupport.pluginDomain + 'trackSession.action?' + WSFlowQueryString.join('&') + similarproducts.utilities.abTestUtil.getDataString();
                }
			},5000);
		...
		puPSearch: function(rep, im){
            if(similarproducts.b.WSFlow){
            	similarproducts.b.WSFlow.addStep("In puPSearch");
            }
			if (rep < 101) {
				var sp = spsupport.p;
							...
							if (sfu) {
								if( sp.prodPage.s < 2 && !sp.prodPage.e){
									...
									if (!(ii == 6 && similarproducts.b.userData.storageData['sf_uninstall_inimg'])){
										sfu.prepareData(ob, 1, ifSg, c1, ii, spsupport.p.$('.__similarproducts.similarproducts_inimg').length, 0, 0, spsupport.p.$(im).outerWidth(), spsupport.p.$(im).outerHeight());
									}
		...
		{
            if(similarproducts.b.WSFlow){
            	similarproducts.b.WSFlow.addStep("in startDOMEnumeration");
            }
			var sfa = spsupport.api;
			...
			if( ss.validRefState() )
			{
				...
				if(similarproducts.b.inimgSrp && spsupport.sites.su() && (!sp.prodPage.p && !sp.prodPage.s || spsupport.sites.isSrp()) && sp.SRP.p.length )
				{
					...
					sp.SRP.lim = similarproducts.b.inimgSrp ?
						similarproducts.b.inimgSrp && similarproducts.b.numOfDisplayedInImgUnits ? similarproducts.b.numOfDisplayedInImgUnits :  similarproducts.b.inimgSrp
						: 0;
